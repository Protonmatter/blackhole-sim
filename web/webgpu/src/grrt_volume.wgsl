...
fn fs_main(in: VSOut) -> @location(0) vec4<f32> {
  ...
  for (var i=0; i<MAX_FRAGMENT_STEPS; i=i+1) {
    ...
    if (yn.r <= horizon(a)*1.0002) { captured = true; col = col*0.03; break; }
    ...
    if (gg > 1e-6) {
      ...
      let dl = U.step_h*0.08/max(gg, 1e-4);
  ...
  if (!captured) {
    ...
    col = col + 0.08 * exp(-min(tau, 18.0)) * pathGain * escapeGain * skyColor(y.th, y.ph, ndc, a);
  }
  col = U.exposure*col;
